...
class ProjectImageTable extends \yii\db\ActiveRecord
{
    ...
    public function afterDelete()
    {
        ...
        if (isset(Yii::$app->params['project']) && is_array(Yii::$app->params['project'])) {
            foreach (Yii::$app->params['project'] as $key => $value) {
                $path = Yii::getAlias('@frontend/web') . $value['folder'];
                $pathSave = $path . $this->image_url;
                if (!is_dir($pathSave) && file_exists($pathSave)) @unlink($pathSave);
            }
        }
